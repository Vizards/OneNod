...
On macOS, also validate the optional Beholder runtime and its kernel process
attribution before changing or releasing it:

```sh
go -C cmd/beholder-core test -race ./...
go -C cmd/beholder-core vet ./...
go -C cmd/beholder-gatekeeper test -race ./...
go -C cmd/beholder-gatekeeper vet ./...
```
